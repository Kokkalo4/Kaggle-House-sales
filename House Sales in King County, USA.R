...
sqrt(mean((actuals_preds$actuals-actuals_preds$predicteds)^2)) *100 / 
  (max(actuals_preds$actuals)-min(actuals_preds$actuals))
library(hydroGOF)
nrmse(actuals_preds$predicteds,actuals_preds$actuals,norm="maxmin")
...
nrmse(test$price,mult.pred,norm="maxmin")
